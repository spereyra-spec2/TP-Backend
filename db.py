...
def obtener_ranking(limit, offset):
    try:
        ...
        query = f"""
            ...
            CAST(SUM(
                CASE 
                    WHEN prediccion.local = partido.equipo_local AND prediccion.visitante = partido.equipo_visitante THEN 3

                    WHEN SIGN(prediccion.local - prediccion.visitante) = SIGN(partido.equipo_local - partido.equipo_visitante) THEN 1

                    ELSE 0 
                END
            ) AS unsigned) AS puntos
        FROM prediccion
        JOIN partido ON prediccion.id_partido = partido.id
        ...
        print(f"Error en obtener_ranking: {e}")
        return None
...
def partido_tiene_resultado(partido_id):
    ...
    try:
        ...
        return partido and (
            partido.get('local') is not None or 
            partido.get('visitante') is not None
        )
    except Exception as e:
